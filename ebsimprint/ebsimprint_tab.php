 ...
 *
 * Affiche :
 * - Les informations de la commande utilisées pour les fichiers .prj
 * - Le bouton de génération (ou regénération) des fichiers
 * - La liste des fichiers déjà générés avec téléchargement et aperçu
 */
...
if (!isModEnabled('ebsimprint')) {
    print '<div class="error">' . $langs->trans('EbsModuleNotEnabled') . '</div>';
    exit;
}
...
// CSS du module
$cssUrl = dol_buildpath('/ebsimprint/css/ebsimprint.css', 1);
print '<link rel="stylesheet" type="text/css" href="' . $cssUrl . '?v=1.0">';
...
foreach ($object->lines as $line) {
    if ($ebs->shouldSkipLine($line)) continue;
...
if ($hasFiles) {
    ...
    print ' <span class="badge badge-status4 badge-status">' . count($existingFiles) . ' ' . $langs->trans('files') . '</span>';
    print '</h3>';
    ...
    foreach ($existingFiles as $filename) {
        ...
        $fileSize = file_exists($filePath) ? dol_print_size(filesize($filePath), 1, 1) : '-';
        ...
        if ($ext === 'prj') {
            ...
            $icon = '<span class="fa fa-file-image-o" style="color:#28a745" title="Aperçu PNG"></span>';
            $type = 'Aperçu PNG';
        }
...
jQuery(document).ready(function($) {
    ...
    $("#btn-ebs-generate").on("click", function() {
        ...
        $btn.prop("disabled", true).html(\'<span class="fa fa-spinner fa-spin paddingright"></span>' . $langs->trans('EbsGenerating') . '\');

        $("#ebs-status")
            .removeClass("error ok")
            .addClass("loading")
            .html(\'<span class="fa fa-spinner fa-spin paddingright"></span>' . $langs->trans('EbsGenerating') . '...\')
            .show();
        ...
        $.ajax({
            ...
            success: function(resp) {
                if (resp.success) {
                    ...
                    // Recharger la page pour afficher les fichiers
                    ...
                    $btn.prop("disabled", false).html(\'<span class="fa fa-print paddingright"></span>' . $langs->trans('EbsGenerate') . '\');
                }
            },
            error: function(xhr) {
                $("#ebs-status")
                    .removeClass("loading ok")
                    .addClass("error")
                    .html(\'<span class="fa fa-times paddingright"></span>Erreur réseau : \' + xhr.statusText);
                $btn.prop("disabled", false).html(\'<span class="fa fa-print paddingright"></span>' . $langs->trans('EbsGenerate') . '\');
            }
        });
    });
    ...
    $("#btn-ebs-delete").on("click", function() {
        if (!confirm("' . $langs->trans('EbsConfirmDelete') . '")) { return; }
